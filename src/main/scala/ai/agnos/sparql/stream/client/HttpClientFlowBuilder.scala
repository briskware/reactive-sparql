...
trait HttpClientFlowBuilder {
  ...
  /**
    ...
    * @return
    */
  def pooledHttpClientFlow[T](endpoint: HttpEndpoint, sslConfig: Option[PekkoSSLConfig] = None)
                             (implicit system: ActorSystem, materializer: Materializer)
  : Flow[(HttpRequest, T), (Try[HttpResponse], T), NotUsed] = {
    endpoint.protocol match {
      case "http" => Flow[(HttpRequest, T)].via(Http().cachedHostConnectionPool(endpoint.host, endpoint.port))
      case "https" =>
        val cfg = sslConfig.getOrElse(PekkoSSLConfig())
        val ctx = Http().createClientHttpsContext(cfg)
  ...
  /**
    ...
    * @param httpsContext HTTPS context
    * @param system
    * @param materializer
    * @tparam T
    * @return
  ...
  (
    ...
    val queue = Source.queue[(HttpRequest, Promise[HttpResponse])](queueSize, overflowStrategy)
      .via(pooledHttpClientFlow[Promise[HttpResponse]](endpoint, sslConfig))
      .toMat(Sink.foreach({
